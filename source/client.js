/*global Buffer:true, Promise:true, require:false, module: false */
(function(module) {
	...
	function processDirectoryResultFilename(path, resultFilename, basePath) {
		//console.log('parameters in processDirectoryResultFilename: ', path, resultFilename, basePath);
		...
		var pos = resultFilename.indexOf(basePath);
		if (pos === 0) {
			//console.log('processDirectoryResultFilename result: ', resultFilename.substr(basePath.length));
			return resultFilename.substr(basePath.length);
		}
		return "";
	}

	function processDirectoryResult(path, dirResult, basePath, targetOnly) {
		var items = [],
				responseItems = [],
				...
				return new Promise(function(resolve, reject) {
					parser.parseString(body, function (err, result) {
						if (err) {
							...
							(resolve)(processDirectoryResult(path, result, getUriPathName(auth.url)));
						}
					});
				});
			});
				...
				return new Promise(function(resolve, reject) {
					parser.parseString(body, function (err, result) {
						if (err) {
							...
							(resolve)(processDirectoryResult(path, result, getUriPathName(auth.url), true));
						}
					});
				});
			});
